...
let ITERS = 0;
const fresh = () => FRESH++;
const reset = () => { SUBST = new Map(); FRESH = 0; ITERS = 0; };

// ---------- Interactions (from the IC spec) ----------
function appLam(lam, arg) {          // (λx.f a) → x<-a; f
  ITERS++; SUBST.set(lam.n, arg); return whnf(lam.b);
}
...
function dupEra(dup) {               // !&L{r,s}=*; K
  ITERS++; SUBST.set(dup.x, Era()); SUBST.set(dup.y, Era()); return whnf(dup.b);
}
...
function dupSup(dup, sup) {          // !&L{x,y}=&R{a,b}; K
  ITERS++;
  if (dup.l === sup.l) {
    SUBST.set(dup.x, sup.a); SUBST.set(dup.y, sup.b);
    return whnf(dup.b);
  }
...
function whnf(term) {
  while (true) {
    switch (term.t) {
      case 'Var': {
        if (SUBST.has(term.n)) { term = SUBST.get(term.n); continue; }
        return term;
      }
      case 'Let': { const v = whnf(term.v); SUBST.set(term.n, v); term = term.b; continue; }
      case 'App': {
        const f = whnf(term.f);
        if (f.t === 'Lam') { term = (ITERS++, SUBST.set(f.n, term.a), f.b); continue; }
        if (f.t === 'Sup') { return appSup(f, term.a); }
        if (f.t === 'Era') { ITERS++; return Era(); }
        if (f.t === 'Dup') { ITERS++; term = Dup(f.l, f.x, f.y, f.v, App(f.b, term.a)); continue; }
        return App(f, term.a);
      }
      case 'Dup': {
        const v = whnf(term.v);
        ...
        if (v.t === 'Dup') { term = Dup(v.l, v.x, v.y, v.v, Dup(term.l, term.x, term.y, v.b, term.b)); continue; }
...
function normal(term) {
  const q = whnf(term);
  switch (q.t) {
    case 'Lam': return Lam(q.n, normal(q.b));
    case 'App': return App(normal(q.f), normal(q.a));
    case 'Sup': return Sup(q.l, normal(q.a), normal(q.b));
    case 'Dup': return Dup(q.l, q.x, q.y, normal(q.v), normal(q.b));
    default: return q;
  }
}

function show(term, names = new Map()) {
  const nm = n => { if (!names.has(n)) names.set(n, 'abcdefghijklmnopqrstuvwxyz'[names.size % 26] + (names.size >= 26 ? Math.floor(names.size / 26) : '')); return names.get(n); };
  switch (term.t) {
    case 'Var': return nm(term.n);
    case 'Era': return '*';
    case 'Lam': return `λ${nm(term.n)}.${show(term.b, names)}`;
    case 'App': return `(${show(term.f, names)} ${show(term.a, names)})`;
    case 'Sup': return `&${term.l}{${show(term.a, names)},${show(term.b, names)}}`;
    case 'Dup': return `!&${term.l}{${nm(term.x)},${nm(term.y)}}=${show(term.v, names)}; ${show(term.b, names)}`;
    default: return '?';
  }
}
...
function nsub(t, n, v) {
  switch (t.t) {
    case 'Var': return t.n === n ? v : t;
    case 'Lam': return t.n === n ? t : Lam(t.n, nsub(t.b, n, v));
    case 'App': return App(nsub(t.f, n, v), nsub(t.a, n, v));
    default: return t;
  }
}
function nwhnf(t, budget) {
  while (true) {
    ...
    if (t.t === 'App') {
      ...
      if (f.t === 'Lam') { BETA++; t = nsub(f.b, f.n, t.a); continue; }
      return App(f, t.a);
    }
    return t;
  }
...
function icDemo(N) {
  ...
  const T = () => { const t = fresh(), fv = fresh(); return Lam(t, Lam(fv, Var(t))); };
  const NOT = () => {
...
for (const N of [4, 8, 16, 24, 30, 100, 1000]) {
  const r = icDemo(N);
  console.log(`N=${N} (2^${N} = ${N <= 30 ? 2 ** N : 'astronomical'} applications): ${r.inters} interactions, ${r.ms.toFixed(2)}ms -> ${r.result}`);
}

console.log('\n=== naive λ-calculus: same computation, one β at a time ===');
for (const N of [4, 8, 12]) {
  const r = naiveDemo(N, 50_000_000);
  console.log(`N=${N} (2^${N} applications): ${r.betas} β-steps, ${r.ms === null ? 'DNF' : r.ms.toFixed(2) + 'ms'} -> ${r.result}`);
}
...
function T_() { const t = fresh(), f = fresh(); return Lam(t, Lam(f, Var(t))); }
function F_() { const t = fresh(), f = fresh(); return Lam(t, Lam(f, Var(f))); }
function NOT_(a) { const t = fresh(), f = fresh(); return Lam(t, Lam(f, App(App(a, Var(f)), Var(t)))).b ? Lam(t, Lam(f, App(App(a, Var(f)), Var(t)))) : null; }
function AND_(a, b) { const x = fresh(), y = fresh(); return App(App(App(a, Lam(x, Var(x))), Lam(y, F_())), b); }
function OR_(a, b) { const x = fresh(), y = fresh(); return App(App(App(a, Lam(x, T_())), Lam(y, Var(y))), b); }

let DUPLAB = 1_000_000;
function clones(term, k, out) {
  // produce k correlated copies of term via chained DUPs (fresh labels)
  if (k === 1) { out.push(term); return (b) => b; }
  let wrap = (b) => b;
  let cur = term;
  for (let i = 0; i < k - 1; i++) {
    const l = DUPLAB++;
...
function satDemo(nVars, clauses) {
  ...
  function isTrue(t) { return t.t === 'Lam' && t.b.t === 'Lam' && t.b.b.t === 'Var' && t.b.b.n === t.n; }
...
function bruteForce(nVars, clauses) {
  ...
  for (let m = 0; m < 2 ** nVars; m++) {
    ...
    for (const cl of clauses) {
      let cv = false;
      for (const [v, neg] of cl) { const val = ((m >> v) & 1) === 1; if (neg ? !val : val) { cv = true; break; } }
...
for (const [n, m, seed] of [[8, 18, 7], [10, 22, 3], [12, 26, 1], [14, 32, 5]]) {
  ...
  const verify = r.sol === null ? 'UNSAT?' : cnf.every(cl => cl.some(([v, neg]) => neg ? !(r.sol[v] ?? true) : (r.sol[v] ?? true)));
  console.log(`n=${n} vars, ${m} clauses: IC ${r.inters} inters ${r.ms.toFixed(1)}ms sol=${r.sol ? 'found' : 'none'} verified=${verify} | JS brute force: ${b.tried} tries ${b.ms.toFixed(1)}ms`);
}

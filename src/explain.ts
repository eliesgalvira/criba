...
const LWORDS: Record<"es" | "en", LWords> = {
  es: {
    start: "empieza: total = 0, y toma tu n",
    repeat: "repite (el total no se borra entre vueltas):",
    eq: (d) => `si n = ${d}`,
    ge: (d) => `si n ≥ ${d}`,
    stop: "párate",
    addTotal: (e) => `suma ${e} al total`,
    again: (e) => `vuelve arriba con n = ${e}`,
    result: "al pararte, el resultado es el total",
    resultIs: (e) => `el resultado es ${e}, tal cual`,
    and: "y",
  },
  en: {
    start: "start: total = 0, and take your n",
    repeat: "repeat (the total carries over):",
    eq: (d) => `if n = ${d}`,
    ge: (d) => `if n ≥ ${d}`,
    stop: "stop",
    addTotal: (e) => `add ${e} to the total`,
    again: (e) => `go back up with n = ${e}`,
